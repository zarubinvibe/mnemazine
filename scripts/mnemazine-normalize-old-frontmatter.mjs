...
// платить за regex. Кими остается то, что действительно требует головы: блоки «Как это поможет мне» и
...
//   • не ставит `подтвержден` там, где проверки не было (для этого владелец завел «не-проверялось»);
//   • не трогает тело ноты вообще;
//   • не создает frontmatter с нуля — нота без него уходит в отчет, ее чинит конвейер.
//
...
// с дублем `type`/`verified` не правится, а уходит в отчет (fail-closed).
 ...
 *   `decision`  — требует непустые `context` и `rejected`, их у старых нот нет: гейт все равно провалит;
 ...
 *      просто потому, что автор дописал скобку — измерено на живом корпусе. Граница обязательна: без нее
 *      «непроверяемо-методомX» тоже сошло бы за валидное значение.
 *   3. да/true/подтверждено… → `подтвержден` (прошлый конвейер именно это и утверждал). Оговорка о
 *      неполноте бьет утверждение сама собой: «частично подтверждено…» начинается не с «подтвержд»,
 *      поэтому сюда не попадает и уезжает в общий случай — отдельная ветка на «частично» была бы мертвым
 *      кодом (проверено мутацией: ее удаление не меняет ни одного исхода).
 *   4. expanded with public sources → `подтвержден`: старый конвейер уже расширил ноту публичными
 ...
 *   6. все прочее — «частично…», даты, свободный текст → `не-проверялось`, оригинал в legacy.
...
export function mapVerified(old) {
  ...
  if (/^(да|true|yes|verified)$/.test(v) || /^подтвержд/.test(v)) return { value: 'подтвержден', keepLegacy: v !== 'подтвержден' }
  if (v === 'expanded with public sources') return { value: 'подтвержден', keepLegacy: true }
...
if (isMain && process.argv.slice(2).includes('--selftest')) {
  ...
  // Оговорка о неполноте бьет слово «подтверждено» — иначе частичная проверка выдаст себя за полную.
  ...
  eq(V('подтверждено на дату предыдущей проверки'), 'подтвержден')
  eq(V('подтвержден'), 'подтвержден')
  eq(V('подтвержден'), 'подтвержден') // е/е — одно слово
  eq(V('true'), 'подтвержден')
  eq(V('expanded with public sources'), 'подтвержден')
  ...
  // «не-проверялось» само себя не ловит префиксом «не» и остается собой.
  ...
  eq(T('tool', { repo: 'x' }), 'concept') // ОДНОГО поля мало: гейт все равно потребует остальные три
...
for (const file of files) {
  ...
  // `source` НЕ выдумывается: берется только первый непустой элемент уже существующего `sources`. Пусто —
