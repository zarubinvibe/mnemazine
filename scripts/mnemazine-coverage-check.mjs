...
// вердикт; в том же прогоне она ещё и чинила собственный баг грепа по ходу проверки. Это generator≈verifier
// на самом важном гейте системы.
//
// Подсчёт покрытия — не суждение, а греп. Значит его место в коде с кодом возврата. LLM остаётся то,
...
// возвращают ровно тот зелёный, что мастер-план §8 запрещает прибору оркестрации.
 ...
 * NFC-нормализация обязательна: macOS отдаёт имена файлов в NFD («й» = «и»+U+0306), а нота пишется из
 * контекста агента в NFC — без выравнивания кириллическое имя даёт ложную дыру на каждом прогоне.
 */
