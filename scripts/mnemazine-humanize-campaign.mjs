...
// git-коммит ПОСЛЕ. Не прошедший гейт вариант отбрасывается — в vault остаётся
...
// Гейт всё равно проверит; промпт лишь повышает шанс пройти.
function humanizePrompt(text) {
  return `Ты очеловечиваешь заметку знаний на РУССКОМ. Верни ВЕСЬ текст заметки целиком, без преамбул и без обёртки в тройные кавычки.
...
async function humanizeNote(text) {
  ...
  // Снять возможную обёртку в тройные кавычки.
...
async function main() {
  ...
  for (const item of batch) {
    ...
    if (g.code === 0) {
      ...
      rewritten += 1
      // приём 24: машинно снимаемый след правки — проза до/после в отчёт партии.
  ...
  await fs.writeFile(report, [
    ...
    '', '## След правки (приём 24)', '', '```json', JSON.stringify(diffs, null, 2), '```'
  ].join('\n') + '\n', 'utf8')

  console.log(JSON.stringify({ mode: 'apply', batch: batch.length, rewritten, kept, report }, null, 2))
  console.error('НАПОМИНАНИЕ (стоп-критерий шаг 8): пересними базовый замер; если после трёх партий медиана не выросла — останови кампанию на разбор.')
  return 0
}
