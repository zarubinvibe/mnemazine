...
// `.mnemazine` — рантайм-состояние, `99 Система/_lint` — отчеты kb-lint. Ни то, ни другое не ноты.
// Набор обязан совпадать с walkVault() в mnemazine-kb-lint.mjs: линтер сравнивает счетчик мастер-индекса
// со своим счетом диска, поэтому любое расхождение НАБОРОВ он печатает как critical «расхождение индексов»
// — вечный красный, который растет с каждым прогоном линта. Измерено 2026-07-26: индексатор считал 1815,
// линтер 1812, разница ровно три отчета в `99 Система/_lint/`. Правишь одну сторону — правь вторую.
...
if (!CHECK) {
  // Файлы, которые этот же прогон создает, считаются существующими — иначе на
  // свежем vault записанные счетчики устаревают в момент записи и --check
...
// Ячейка счетчика — третья колонка таблицы разделов; строка узнается по тому,
...
// ── Запись: пересчет мастер-индекса, индекса Capabilities, запись в лог ──

const labels = {
  '00 Входящие': 'сырье и временные буферы',
...
Кратко: отдельная структура для Skills, MCP, plugins, agents и workflow. Методика сохраняет правила установки/оценки; реестр дает полный каталог; конкретные знания ссылаются на применимые способности.
...
const heading = `## [${today}] индекс | Пересчет мастер-индекса и Capabilities`
const entryLines = [
  heading,
